...
<div class="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-2">
    <div>
        <h1 class="h3 mb-0" style="font-weight:700;color:#111827;">Rekap Surat Unit</h1>
        <p class="text-muted mb-0" style="font-size:.85rem;">
            <i class="fas fa-building me-1"></i>
            <?= esc($unitKerja['nama_unit_kerja'] ?? 'Unit Kerja Tidak Diketahui') ?>
        </p>
    </div>
    <a href="<?= base_url('tu-unit/dashboard') ?>" class="btn btn-outline-secondary btn-sm px-3">
...
<div class="card shadow-sm mb-4" style="border-radius:16px; border: 1px solid #e5e7eb;">
    <div class="card-body p-3">
        <form method="GET">
            <!-- Baris 1: Filter tanggal, tahun, jenis, status -->
            ...
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto d-flex gap-2">
                    ...
                    <a href="<?= base_url('tu-unit/rekap') ?>" class="btn btn-outline-secondary btn-sm px-3">
                ...
                <table class="custom-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-size:.82rem;"><?= esc($namaTampil) ?></td>
                ...
                <?php if ($p['total_pages'] > 1): ?>
                    <div class="numbered-pagination">
                        ...
                        <?php if ($currentPage > 1): ?>
                            <a class="page-btn" href="?page=<?= $currentPage - 1 ?>&start_date=<?= urlencode($startDate??'') ?>&end_date=<?= urlencode($endDate??'') ?>&tahun=<?= $tahun ?>&jenis=<?= urlencode($jenisDokumen) ?>&status=<?= $status ?>&search=<?= urlencode($search) ?>">&laquo;</a>
                        <?php endif ?>

                        <?php if ($start > 1): ?>
                            <a class="page-btn" href="?page=1&start_date=<?= urlencode($startDate??'') ?>&end_date=<?= urlencode($endDate??'') ?>&tahun=<?= $tahun ?>&jenis=<?= urlencode($jenisDokumen) ?>&status=<?= $status ?>&search=<?= urlencode($search) ?>">1</a>
                        ...
                        <?php for ($pg = $start; $pg <= $end; $pg++): ?>
                            <a class="page-btn <?= $pg === $currentPage ? 'active' : '' ?>"
                               href="?page=<?= $pg ?>&start_date=<?= urlencode($startDate??'') ?>&end_date=<?= urlencode($endDate??'') ?>&tahun=<?= $tahun ?>&jenis=<?= urlencode($jenisDokumen) ?>&status=<?= $status ?>&search=<?= urlencode($search) ?>">
                                <?= $pg ?>
                            </a>
                        <?php endfor; ?>
                        ...
                        <?php if ($end < $totalPages): ?>
                            ...
                            <a class="page-btn" href="?page=<?= $totalPages ?>&start_date=<?= urlencode($startDate??'') ?>&end_date=<?= urlencode($endDate??'') ?>&tahun=<?= $tahun ?>&jenis=<?= urlencode($jenisDokumen) ?>&status=<?= $status ?>&search=<?= urlencode($search) ?>"><?= $totalPages ?></a>
                        <?php endif; ?>

                        <?php if ($currentPage < $totalPages): ?>
                            <a class="page-btn" href="?page=<?= $currentPage + 1 ?>&start_date=<?= urlencode($startDate??'') ?>&end_date=<?= urlencode($endDate??'') ?>&tahun=<?= $tahun ?>&jenis=<?= urlencode($jenisDokumen) ?>&status=<?= $status ?>&search=<?= urlencode($search) ?>">&raquo;</a>
                        <?php endif ?>
                    </div>
                <?php endif ?>
            </div>
